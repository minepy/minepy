...
starting from the pseudocode provided in DOI: 10.1126/science.1205438,
Supplementary On-line Material (SOM), as no original Java source code is
available. The level of detail of the pseudocode leaves a few ambiguities and in
...
as well as for difference in performance. See DOI:
...
#. The function GetClumpsPartition(:math:`D,Q`) is discussed
   (SOM page 12), but its pseudocode is not explicitely
...
#. We also explicitly provide the pseudocode for the
   GetSuperclumpsPartition() function (SOM
...
#. We observed that the GetSuperclumpsPartition() implemented in
   ...
   (:math:`\alpha=0.551`, :math:`c=10`) used in the original work to the
...
#. In order to speed up the computation of the MINE statistics, we
   ...
   OptimizeXAxis(), defined in Algorithm 2 in SOM):
